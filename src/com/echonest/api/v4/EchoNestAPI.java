...
public class EchoNestAPI {
    ...
    public enum TermType { ANY, STYLE, MOOD };
   ...
   /**
     ...
     *
     * @param p the params for the playlist (see PlaylistParams)
     * @return the playlist
     * @throws EchoNestException
     */
    @SuppressWarnings("unchecked")
     ...
     * @param rating a rating of 1 to 5 (zero means no rating)
     * @return the playlist
     * @throws EchoNestException
     */
    @Deprecated
     ...
     * @throws EchoNestException
     */
    public String dynamicPlaylistCreate(Params p) throws EchoNestException {
     ...
     * @throws EchoNestException
     */
    public String dynamicPlaylistRestart(String sessionID, Params p) throws EchoNestException {
        p.set("session_id", sessionID);
        Map results = cmd.sendCommand("playlist/dynamic/restart", p);
        Map response = (Map) results.get("response");
        return sessionID;
    }
     ...
     *
     * @param sessionID the session of interest
     * @param results the number of songs that should be returned
     * @param lookahead the number of lookahead songs to return
     *
     * @return the playlist
     *
     * @throws EchoNestException
     */
    public Playlist dynamicPlaylistNext(String sessionID, int results, int lookahead) throws EchoNestException {
        Params p = new Params();

        p.set("result", results);
        p.set("lookahead", lookahead);
        p.set("session_id", sessionID);

        Map cresults = cmd.sendCommand("playlist/dynamic/next", p);
        ...
        for (int i = 0; i < lookaheadList.size(); i++) {
            Song song = new Song(this, (Map) lookaheadList.get(i));
    ...
    public void dynamicPlaylistFeedback(String sessionID, Params p) throws EchoNestException {
        p.set("session_id", sessionID);
        Map results = cmd.sendCommand("playlist/dynamic/feedback", p);
    }

    /**
     * Steers the playlist based upon the given parameters. Steerings are
     * additive.
     *
     * @param sessionID the session id of interest
     * @param p
     * @throws EchoNestException
     */
    public void dynamicPlaylistSteer(String sessionID, Params p) throws EchoNestException {
        p.set("session_id", sessionID);
        Map results = cmd.sendCommand("playlist/dynamic/steer", p);
    }

    /**
     * Returns info about a dynamic playlist
     *
     * @param sessionID the session id of interest
     *
     * @throws EchoNestException
     */
    public Map dynamicPlaylistInfo(String sessionID) throws EchoNestException {
        Params p = new Params();
        p.set("session_id", sessionID);
        Map cresults = cmd.sendCommand("playlist/dynamic/info", p);
        Map response = (Map) cresults.get("response");
        return response;
    }

    /**
     * Deletes a previously created playlist session
     *
     * @param sessionID the session id
     * @throws EchoNestException
     */
    public void dynamicPlaylistDelete(String sessionID) throws EchoNestException {
        Params p = new Params();
        p.set("session_id", sessionID);
        Map cresults = cmd.sendCommand("playlist/dynamic/delete", p);
        Map response = (Map) cresults.get("response");
    }
